...
export function buildJudgePrompt(ctx: JudgePromptContext): string {
  ...
  return [
    ...
    `**Tests are mandatory, not optional.** This project is test-driven: the work is **NOT met** unless EVERY acceptance criterion is covered by an automated test that genuinely exercises the new behaviour — one that would FAIL without the implementation. Confirm the diff actually adds or updates such tests (the repo's own framework; a runner should be set up if none existed). Treat missing tests, tests that don't assert the new behaviour, and trivial/tautological/assertion-free tests as NOT met, and say exactly which criteria still need real test coverage. The ONLY exception is a change with genuinely nothing to assert (e.g. docs or static assets).`,
