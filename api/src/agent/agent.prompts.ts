...
1. At session start, call \`memory_get(jobId="${ctx.jobId}", prefix="plan:")\` to check for existing progress (resumed session).
2. If fresh, call \`memory_set(jobId="${ctx.jobId}", key="plan:<filepath>", value="pending")\` for each file in the plan.
3. As you complete each file, call \`memory_set(jobId="${ctx.jobId}", key="plan:<filepath>", value="done")\`.
4. After any context compaction, immediately call \`memory_get(jobId="${ctx.jobId}", prefix="plan:")\` to recover your full progress list before continuing.
5. Before ending, call \`memory_get\` and confirm every entry shows \`"done"\` — if any are still \`"pending"\`, complete them first.`,
    `**Codebase exploration** — if \`memory_get\` returned no existing progress (fresh session), before writing any files, run a read-only exploration subagent to locate the files you will need:
\`\`\`
delegate_task(
