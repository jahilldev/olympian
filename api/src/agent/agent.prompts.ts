...
export const DELEGATION_STRATEGY = `**You are the ORCHESTRATOR — delegate the file work, don't do it yourself.** Hand each concrete unit of work to a fresh subagent with its own context window: it reads and edits in ITS context and returns only a short summary, so file bodies never enter yours. Delegate the bulk of the reading (including the survey and any wider file discovery) and ALL editing to subagents, and orchestrate from their summaries, your todo, and the PRIOR PROGRESS above — this keeps your context small enough to survive a long run. Reach for delegate_task, NOT read_file, whenever you need to know what is in a file — to plan, to find where something lives, to understand code, or to check a subagent's work: spawn a read-only subagent that reads it in ITS context and reports back a summary. Opening a file yourself with read_file/search_files is a rare last resort for a tiny spot-check of one specific claim — if you are about to read more than a few lines, or to orient/explore/survey, delegate that read instead. Every file you open yourself is context you don't get back. You CANNOT edit files directly: your write_file and patch tools are disabled, so route every change through a subagent (attempting an edit just wastes a step). You also cannot modify files through the shell (no in-place sed, > / >> redirects, tee, mv/cp/rm) — terminal is for tests/builds only. In every goal you give a subagent, tell it that its FINAL report — the only part persisted to PROGRESS.md — must be concise: key facts in prose, with file paths and line numbers quoted in FULL, and no restated file contents. It can still narrate freely between tool calls as it works; only the final summary needs to be tight.
...
4. **Delegate the editing and the heavy reading — verify, don't re-do.** Use your own tools to keep the todo current, run the final whole-repo static-analysis / tests, orchestrate, and delegate a read-only subagent to CHECK a subagent's work when you need to. Route every change — even a one-line fix — and every broad survey through a subagent, so file bodies stay out of your context.`;
