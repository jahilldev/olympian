...
export const WORKING_MEMORY_CONTRACT = `**Keep durable working notes in \`.olympian/PROGRESS.md\` — your primary source of truth, ahead of the filesystem.** It is the ONE record that survives everything: context compaction AND a crashed or early-exiting run. Anything you will need later — in this run or a follow-up run resuming the same task — must live on disk here:
...
- **Update PROGRESS.md the INSTANT an item is done — in the same step, before you touch the next item.** Tick it (\`- [x]\`) and note where the change landed. NEVER batch these updates for the end of the session, and NEVER add "update PROGRESS.md" as a checklist item of its own — keeping the file current IS part of completing each item, not a separate task. If you defer it and the run is interrupted (error, early exit, or compaction), that progress is lost, which is the exact failure this file exists to prevent.
- If you also track work with the in-context todo tool, PROGRESS.md is its DURABLE mirror — update both together, item by item. The todo list is re-injected through compaction, but it is per-run: a fresh run (after an error or early exit) starts with it EMPTY, it drops completed items, and it never holds your findings. PROGRESS.md is on disk, so it survives into the next run and keeps the file/line notes, decisions, and completed history the todo list can't — it is the source of truth.
