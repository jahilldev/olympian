...
export const DELEGATION_STRATEGY = `**You are the ORCHESTRATOR — delegate the file work, don't do it yourself.** Hand each concrete unit of work to a fresh subagent with its own context window: it reads and edits in ITS context and returns only a short summary, so file bodies never enter yours. Delegate the bulk of the reading (including the survey and any wider file discovery) and ALL editing to subagents, and orchestrate from their summaries, your todo, and the PRIOR PROGRESS above — this keeps your context small enough to survive a long run. For broad work — surveying the codebase, searching for where something lives, or reading several files — spawn a subagent; only read a file directly when you need to verify or spot-check a specific subagent result. You CANNOT edit files directly: your write_file and patch tools are disabled, so route every change through a subagent (attempting an edit just wastes a step). You also cannot modify files through the shell (no in-place sed, > / >> redirects, tee, mv/cp/rm) — terminal is for tests/builds only. In every goal you give a subagent, tell it to report CONCISELY: key facts in prose only, with file paths and line numbers quoted in FULL, and never restate file contents.
...
4. **Delegate the editing and the heavy reading — verify, don't re-do.** Use your own tools to keep the todo current, run the final whole-repo static-analysis / tests, orchestrate, and read a file or two to CHECK a subagent's work. Route every change — even a one-line fix — and every broad survey through a subagent, so file bodies stay out of your context.`;
