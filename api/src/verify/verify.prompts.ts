...
export function buildVerifyPrompt(ctx: VerifyPromptContext): string {
  return [
    `You are Hermes working in a clone of \`${ctx.repoFullName}\`. Your current working directory IS the repo root — the full repository is already checked out here. Do NOT clone, fetch, pull, or browse GitHub; do NOT use git commands. Just read the files that are already present. Determine the single shell command that best verifies whether a change to this repository is correct.`,
