import type { ReviewPassDto } from "@olympian/api/interface/interface.model.js";
      ...
      {pass.issues.length > 0 && (
        <div class="space-y-2 pt-1">
          {pass.issues.map((issue, i) => (
            <div
              key={i}
              class="rounded border border-zinc-700 bg-zinc-800/50 p-3 space-y-1"
            >
              ...
              {issue.file && (
                <p class="text-xs text-zinc-500 font-mono">{issue.file}</p>
              )}
            </div>
          ))}
        </div>
