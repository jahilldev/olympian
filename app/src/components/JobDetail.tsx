...
import type {
  ...
  FeedbackDto,
  ReviewPassDto,
} from "@olympian/api/interface/interface.model.js";
  ...
  return (
    <div class="space-y-2">
      {items.map((f) => (
        <div
          key={f.id}
          class="rounded-lg border border-zinc-800 bg-zinc-900 p-3 space-y-1"
        >
          <div class="flex items-center justify-between text-xs text-zinc-500">
            <span class="font-mono text-zinc-400">@{f.author}</span>
            <span>{relativeTime(f.createdAt)}</span>
          </div>
          <p class="text-sm text-zinc-300 leading-relaxed whitespace-pre-wrap">
            {f.body}
          </p>
        </div>
      ))}
...
export default function JobDetail({ id }: Props) {
  ...
  useEffect(() => {
    ...
    async function fetchJob() {
      try {
        const res = await fetch(`/interface/jobs/${id}`);
        if (res.status === 404) {
    ...
    void fetchJob();
    timer = setInterval(() => void fetchJob(), 2_000);
    ...
    <div class="flex flex-col h-full overflow-hidden">
      ...
      <div class="flex-1 overflow-hidden flex flex-col lg:flex-row">
        ...
        <div class="lg:w-3/5 overflow-y-auto p-6 space-y-6 border-b lg:border-b-0 lg:border-r border-zinc-800">
          {/* Title + badge */}
          <div class="space-y-2">
            <h1 class="text-xl font-semibold text-zinc-100 leading-snug">
              {job.issueTitle}
            </h1>
        ...
        <div class="lg:w-2/5 overflow-y-auto p-6 space-y-6">
          {/* Review cycles */}
          {job.reviewPasses.length > 0 && (
            <div class="space-y-2">
              <h3 class="text-xs font-medium text-zinc-500 uppercase tracking-wide">
                Reviews
              </h3>
              <ReviewCycles passes={job.reviewPasses} />
            </div>
          )}
          ...
          {job.runs.length > 0 && (
            <div class="space-y-1">
              <h3 class="text-xs font-medium text-zinc-500 uppercase tracking-wide mb-2">
                Runs
              </h3>
              {job.runs.map((run) => (
